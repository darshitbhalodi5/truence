...
export function Management({ walletAddress }: { walletAddress?: string }) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[200px]">
        <LoadingSpinner text="Fetching latest submissions for management" />
      </div>
    );
  }
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-white/80">
          You are not assigned as a manager for any bounties.
        </p>
      </div>
    );
  }
  ...
  if (!managerData.submissions || managerData.submissions.length === 0) {
    return (
      <div className="text-center py-12">
        <h3 className="text-xl font-semibold text-[#FAFCA3] mb-2">
          No Submissions to Manage
        </h3>
        <p className="text-white/80">
          There are no submissions to manage at this time.
        </p>
      </div>
    );
  }
